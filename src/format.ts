import { User, type Message } from "discord.js";

export function emojiID(name: string): string {
	switch (name) {
		case "acwarning":
			return "1176594884711436318";

		case "accooldown":
			return "1176594872086573097";

		case "acerror":
			return "1176594875190362132";

		case "acinfo":
			return "1176594877056815256";

		case "acok":
			return "1176594880219336704";

		case "actop":
			return "1176594881968341196";

		case "acscream":
			return "1176604206409465936";

		case "acplus":
			return "1177236771864592444";

		case "acminus":
			return "1177236768370741340";

		case "acdumbbells":
			return "1177261206873063527";

		case "loading":
			return "1193949801801322496";

		case "minecraft_clock":
			return "1328650986385969152";

		case "resting":
			return "1363928478109405314";

		case "ganyuroll":
			return "1466476428697665599";
	}

	return `0`;
}

export function emoji(name: string | number, animated: boolean = false): string {
	if (typeof name === "number") {
		return [":zero:", ":one:", ":two:", ":three:", ":four:", ":five:",
				":six:", ":seven:", ":eight:", ":nine:"][name] as string;
	}

	return (animated)
		? `<a:${name}:${emojiID(name)}>`
		: `<:${name}:${emojiID(name)}>`;
}

/**
 * Make message content bold.
 */
export function bold(content: string): string {
	return `**${content}**`;
}

/**
 * Make message content underline.
 */
export function underline(content: string): string {
	return `__${content}__`;
}

/**
 * Make message content italic.
 */
export function italic(content: string): string {
	return `*${content}`;
}

/**
 * Make message content code.
 */
export function code(content: string, padding: boolean = false): string {
	return (padding) ? `\` ${content} \`` : `\`${content}\``;
}

/**
 * Make message content heading 1.
 */
export function h1(content: string): string {
	return `# ${content}`;
}

/**
 * Make message content heading 2.
 */
export function h2(content: string): string {
	return `## ${content}`;
}

/**
 * Make message content heading 3.
 */
export function h3(content: string): string {
	return `### ${content}`;
}

/**
 * Make message content sub-heading.
 */
export function sh(content: string): string {
	return `-# ${content}`;
}

/**
 * Multiline message.
 */
export function lines(...lines: (string | null | undefined)[]): string {
	return lines
		.filter((i) => (i !== null && i !== undefined))
		.join("\n");
}

/**
 * Format currency number.
 */
export function money(number: number, { uline = false }: { uline?: boolean } = {}): string {
	const formatter = new Intl.NumberFormat("vi-VN", {
		style: "decimal",
		minimumFractionDigits: 0,
		maximumFractionDigits: 0
	});

	return (uline)
		? `${bold(underline(code(formatter.format(number))))} ${emoji("dongnote")}`
		: `${bold(code(formatter.format(number)))} ${emoji("dongnote")}`;
}

/**
 * Format timestamp to discord date message.
 *
 * @param	format		See {@link https://gist.github.com/LeviSnoot/d9147767abeef2f770e9ddcd91eb85aa}
 */
export function timestampMessage(date: number | Date, format: "" | "t" | "T" | "d" | "D" | "f" | "F" | "R" = "f"): string {
	if (date instanceof Date)
		date = (date.getTime() / 1000);

	return `<t:${Math.floor(date)}${format ? `:${format}` : ""}>`;
}

export function trim(string: string, char: string): string {
	let start = 0,
		end = string.length;

	while (start < end && string[start] === char)
		++start;

	while (end > start && string[end - 1] === char)
		--end;

	return (start > 0 || end < string.length)
		? string.substring(start, end)
		: string;
}

/**
 * Parse message content as arguments.
 */
export function parseArguments(message: Message<boolean>): (string | User | undefined)[] {
	const args: (string | User | undefined)[] = [];

	let tokens = message.content.split(" ");
	let quote: string | null = null;
	let quoteContent = "";

	for (let token of tokens) {
		if (token.startsWith("<@")) {
			let reg = /\<\@(\d+)\>/.exec(token);
			if (reg)
				args.push(message.mentions.users.get(reg[1] as string));
			continue;
		}

		if (token.endsWith("\"")) {
			if (quote) {
				quoteContent += (" " + trim(token, quote));
				args.push(quoteContent);

				quoteContent = "";
				quote = null;
				continue;
			}
		} else if (token.startsWith("\"")) {
			if (!quote) {
				quote = "\"";
				quoteContent = trim(token, "\"");
				continue;
			}
		}

		if (!token || token.length === 0)
			continue;

		if (quote) {
			quoteContent += ` ${token}`;
			continue;
		}

		args.push(token);
	}

	return args;
}

/**
 * Mention a user.
 */
export function mention(user: string | User): string {
	if (user instanceof User)
		user = user.id;

	return `<@${user}>`;
}

export function space(): string {
	return "‎ ‎ ‎ ‎ ‎ ‎ ";
}

export function formatTime(seconds: number): string {
	if (seconds < 0)
		return "Are you trying to break the space-time continuum, you dumbass?";

	const minutes = Math.floor(seconds / 60);
	const hours = Math.floor(minutes / 60);

	const formattedTime: string[] = [];

	if (hours > 0) {
		formattedTime.push(hours + "h");
		seconds -= hours * 3600;
	}

	if (minutes > 0 || hours > 0) {
		formattedTime.push((Math.floor(seconds / 60) % 60) + "m");
		seconds -= minutes * 60;
	}

	if (seconds > 0 || minutes > 0 || hours > 0)
		formattedTime.push(seconds.toFixed(0) + "s");

	const milliseconds = Math.round((seconds - Math.floor(seconds)) * 1000);
	formattedTime.push(milliseconds + "ms");

	return formattedTime.join(" ");
}
