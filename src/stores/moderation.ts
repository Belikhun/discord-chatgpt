...
export type ModerationActionType = "warning" | "kick" | "ban" | "delete_messages" | "clear_warnings";
...
export interface ModerationProfile {
	...
	kickCount: number;
	banCount: number;
	lastWarningExpiresAt: number | null;
	lastAction: { action: string; reason: string | null; createdAt: number | null } | null;
	actions: ModerationActionEntry[];
}

interface GuildModerationStore {
	profiles: Record<string, ModerationProfile>;
	history: ModerationActionEntry[];
}

type ModerationStore = Record<string, any>;

export function ensureModerationStore(): void {
...
export function saveModerationStore(store: ModerationStore): void {
	ensureModerationStore();
	fs.writeFileSync(moderationFilePath, JSON.stringify(store, null, 2), "utf8");
}

function createEmptyProfile(userId: string | null): ModerationProfile {
	return {
		userId,
		warningCount: 0,
...
function createEmptyGuildStore(): GuildModerationStore {
	return {
		profiles: {},
		history: []
	};
...
function normalizeProfile(userId: string | null, profile: Partial<ModerationProfile> = {}): ModerationProfile {
	const normalized: ModerationProfile = {
	...
	normalized.totalWarningCount = Number.isFinite(normalized.totalWarningCount)
		...
		: Number.isFinite(profile.warningCount as number)
			? profile.warningCount as number
	...
	if (!normalized.lastAction && normalized.actions.length > 0) {
		...
		normalized.lastAction = {
			action: (latest as ModerationActionEntry).action,
			reason: (latest as ModerationActionEntry).reason || null,
			createdAt: (latest as ModerationActionEntry).createdAt || null
		};
	}
...
function getWarningExpirationTimestamp(entry: ModerationActionEntry | undefined): number | null {
...
function getForgivenWarningIds(actions: ModerationActionEntry[] = []): Set<string> {
	const forgivenIds = new Set<string>();
...
function listActiveWarningEntries(profile: ModerationProfile, now: number = Date.now()): ModerationActionEntry[] {
...
function isWarningEntryActive(entry: ModerationActionEntry | undefined, now: number = Date.now()): boolean {
	if (!entry || entry.action !== "warning")
		return false;

	const expiresAt = getWarningExpirationTimestamp(entry);
	if (!Number.isFinite(expiresAt as number))
		return false;

	return (expiresAt as number) > now;
}

function normalizeGuildStoreEntry(entry: any = {}): GuildModerationStore {
	if (!entry || typeof entry !== "object")
		return createEmptyGuildStore();

	const normalized: GuildModerationStore = (entry.profiles || entry.history)
		? {
...
function getGuildStore(store: ModerationStore, guildId: string): GuildModerationStore {
	store[guildId] = normalizeGuildStoreEntry(store[guildId]);
	return store[guildId];
}

export function getModerationProfile(guildId: string | null | undefined, userId: string | null | undefined): ModerationProfile {
	if (!guildId || !userId)
...
export interface RecordModerationOptions {
	action: ModerationActionType;
	reason?: string | null;
	actorId?: string | null;
	channelId?: string | null;
...
export function recordModerationAction(guildId: string | null | undefined, userId: string | null | undefined, {
	action,
	reason,
	actorId,
	channelId,
	...
	const profile = userId
		...
		: null;
	const now = Date.now();
	const entry: ModerationActionEntry = {
...
export function clearUserWarnings(guildId: string | null | undefined, userId: string | null | undefined, {
	reason,
	actorId,
	channelId,
	messageId,
	messageIds,
	metadata
}: Omit<RecordModerationOptions, "action"> = {}) {
	if (!guildId || !userId)
...
export function listGuildModerationProfiles(guildId: string, {
	actionFilter,
	limit
}: {
...
export function getModerationHistory(guildId: string, {
	userId,
	action,
	limit
}: {
